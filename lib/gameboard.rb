class Gameboard
  ...
  def check_rows
    @board.each { |row| return row[0] if winning_line?(row) }
    nil
  end
  ...
  def check_columns
    (0...3).each do |col|
      ...
      return column[0] if winning_line?(column)
    end
    nil
  end
  ...
  def check_diagonals
    ...
    return diagonal1[0] if winning_line?(diagonal1)

    diagonal2 = (0...3).map { |i| @board[i][2 - i] }
    return diagonal2[0] if winning_line?(diagonal2)
